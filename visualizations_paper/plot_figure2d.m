% this file generates figures for pre start and post start accuracy for all
% the areas. 
% Plots figure 2d of the manuscript. 
...
for jj = 1:length(brainArea)
    ...
    stop_bf  = stop_accrs(:,before_accr_bins);
    stop_bf = mean(stop_bf,2);

    stop_aft = stop_accrs(:,after_accr_bins);
    stop_aft = mean(stop_aft,2);

    stop_during = stop_accrs(:,during_comparison_bins);
...
for ptid = [1,3]
    ...
    % qt_level_up = qt_level_up - meanprestartaccr' ;
    qt_level_down = get_dstr_based_CI(tmp_dat_all{ptid},100 - qt_val);
    % qt_level_down = meanprestartaccr' - qt_level_down;
    xvalues= (1:numel(X));
    ...
    if ptid == 3
        ylim([min(abs(ylims_here))*0.8,0.4]);
