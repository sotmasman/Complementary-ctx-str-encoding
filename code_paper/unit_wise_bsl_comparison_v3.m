...
for kk = 1:n_units_now
    ...
    this_jitIvals_start = unit_wise_Ivals{kk,1};
    for ll = baseline_bins_str(1):total_bins
        Ival_this = mean(tmp1(:,ll) - this_bsl)/(mean(tmp1(:,ll) + this_bsl));
        this_pvals(ll) = length(find(this_jitIvals_start(:,(ll-baseline_bins_str(1)+1)) > Ival_this))/numberofjitters;
    end
    ...
    this_pvals = ones(1,total_bins);    
    this_jitIvals_stop = unit_wise_Ivals{kk,2};

    for ll = 1:baseline_bins_stp(end)
        Ival_this = mean(tmp1(:,ll) - this_bsl)/(mean(tmp1(:,ll) + this_bsl));
        this_pvals(ll) = length(find(this_jitIvals_stop(:,ll) > Ival_this))/numberofjitters;   
    end
